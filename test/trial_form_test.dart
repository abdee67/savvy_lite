...
import '../lib/models/trial_form_data.dart';
import '../lib/utils/form_validators.dart';
import '../lib/widgets/language_toggle.dart';
import '../lib/widgets/custom_text_field.dart';
import '../lib/services/trial_service.dart';
...
void main() {
  ...
  group('FormValidators Tests', () {
    ...
    test('should validate password correctly', () {
      ...
      expect(FormValidators.validatePassword('12345', true), isNotNull); // Too short
    ...
    test('should validate confirm password correctly', () {
      expect(FormValidators.validateConfirmPassword('password', 'password', true), null);
      expect(FormValidators.validateConfirmPassword('different', 'password', true), isNotNull);
      expect(FormValidators.validateConfirmPassword('', 'password', true), isNotNull);
    });
    ...
    test('should validate username correctly', () {
      ...
      expect(FormValidators.validateUsername('ab', true), isNotNull); // Too short
      expect(FormValidators.validateUsername('user@name', true), isNotNull); // Invalid chars
  ...
  group('TrialService Tests', () {
    ...
    test('should check email availability', () async {
      final isAvailable = await trialService.isEmailAvailable('new@example.com');
      final isUnavailable = await trialService.isEmailAvailable('test@example.com');
  ...
  group('Widget Tests', () {
    testWidgets('LanguageToggle should toggle language', (WidgetTester tester) async {
      bool isEnglish = true;
    ...
    testWidgets('CustomTextField should display correctly', (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(

...
class PurchaseOrderBloc extends Bloc<PurchaseOrderEvent, PurchaseOrderState> {
  ...
  Future<void> _onPrepareCopy(
    ...
    try {
      ...
      final copiedHeader = originalHeader.copyWith(
        ...
        dateTransaction: DateTime.now(),
        dateUpdated: DateTime.now(),
        poReceiveStatus: await _getUdcDetailId('PR', 'N'),
      ...
      final poReceiveStatus = await _getUdcDetailId('PR', 'N');
  ...
  Future<PurchaseOrderHeader> _processHeaderBusinessLogic(
    ...
    final notReceivedStatus = await _getUdcDetailId('PR', 'N');
    final notPaidStatus = await _getUdcDetailId('PS', 'N');
  ...
  Future<void> _onSavePurchaseOrder(
    ...
    try {
      ...
      if (state.autoReceipt == true) {
        final autoReceivers = state.createDetails.map((detail) {
          return PurchaseOrderReceiver(
            ...
            quantityOpen: detail.quantityOpen ?? detail.quantityTransaction,
            amountOpen: detail.amountOpen ?? detail.amountExtendedCost,
            quantityRecieved: detail.quantityTransaction,
            company: savedHeader.company,
            dateReceived: state.receivingDates ?? DateTime.now(),
            userId: state.userId,
            dateUpdated: DateTime.now(),
            //  branchRecieved: savedHeader.branchReceive,
            unitOfMeasure: detail.unitOfMeasure,
            // location: detail.itemLocationsSelect,
            tempId: detail.tempId, // Use detail's tempId
          );
        }).toList();
  ...
  void _onCalculateExtendedCost(
    ...
    if (detail.quantityTransaction != null && detail.unitCost != null) {
      ...
      final index = state.createDetails.indexWhere(
        (d) => d.tempId == detail.tempId || d.id == detail.id,
      );
      ...
      if (event.detail.itemNumber != null) {
        ...
        if (item != null && event.detail.unitOfMeasure == null) {
          ...
          if (uomUdc.isNotEmpty) {
            ...
            // Update in state
            final index = state.createDetails.indexWhere(
              (d) => d.tempId == event.detail.tempId || d.id == event.detail.id,
            );
  ...
  Future<void> _onPrepareReceiptForDetail(
    ...
    try {
      if (event.detail == null) {
        emit(state.errorState('No purchase order detail provided'));
        return;
      }
    ...
    try {
      ...
      for (final receiver in state.editReceivers) {
        ...
        if (receiver.id == null) {
          ...
          // ⭐⭐ UPDATE STOCK (SAME AS JAVA) ⭐⭐
          if (state.selectedHeader?.orderNumber != null) {
            await stockService.updateStockItemAvailabilityPor(
              receiver: savedReceiver,
              companyId: state.companyId!,
              orderNumber: state.selectedHeader!.orderNumber!,
            );
          }
  ...
  Future<void> _updateDetailAndHeaderAfterReceipt({
    ...
    final updatedDetail = detail.copyWith(
      ...
      poReceiveStatus: quantityOpen == 0
          ? await _getUdcDetailId('PR', 'R')
          : await _getUdcDetailId('PR', 'P'),
      dateUpdated: DateTime.now(),
      userId: state.userId,
    );
  ...
  Future<void> _onSaveAndAddNew(
    ...
    if (state.companyId != null) {
      add(PrepareCreatePurchaseOrder(companyId: state.companyId!));
    }
  }
  ...
  void _onRefreshPurchaseOrders(
    ...
    if (state.companyId != null) {
      add(LoadPurchaseOrders(companyId: state.companyId!));
      add(LoadPurchaseOrderCredit(companyId: state.companyId!));
    }
  }

...
class _SupplierEntryScreenState extends State<SupplierEntryScreen> {
  ...
  void initState() {
    ...
    context.read<UdcDetailsBloc>().add(
      const LoadUdcDetailsByGroup('CNTRY'),
  ...
  Widget _buildCountryField() {
    return BlocBuilder<UdcDetailsBloc, UdcDetailsState>(
      builder: (context, state) {
        ...
        String? currentCountryDesc;
        if (_selectedCountry != null) {
          final match = countryList.where(
            (c) => c.description1 == _selectedCountry,
          );
          if (match.isNotEmpty) {
            currentCountryDesc = match.first.description1;
          }
        }
          ...
          items: countryList
              .map(
                (country) => DropdownMenuItem(
                  value: country.description1,
                  child: Text(country.description1 ?? ''),
                ),
              )
              .toList(),
          value: currentCountryDesc,
          onChanged: (value) {
            setState(() {

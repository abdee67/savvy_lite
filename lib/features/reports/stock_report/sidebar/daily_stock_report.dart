...
class DailyStockReport extends StatefulWidget {
  final AuthBloc authBloc;
  const DailyStockReport({super.key, required this.authBloc});

  @override
  State<DailyStockReport> createState() => _DailyStockReportState();
}

class _DailyStockReportState extends State<DailyStockReport>
  ...
  Widget _builditemDetailContent(ItemEntryModel item, bool isCompact) {
    return BlocBuilder<ItemTransactionsBloc, ItemTransactionsState>(
      builder: (context, state) {
        final receivedQty = state.openingQuantityBeforeToday;
        final balanceQty = state.openingQuantityBefore;
        final totalQty = receivedQty + balanceQty;
        final salesQty = state.salesQtyOnDate;
        final remainingQty = state.differenceSalesQty;

...
class PurchaseTransactionReportPage extends StatefulWidget {
  final AuthBloc authBloc;
  const PurchaseTransactionReportPage({super.key, required this.authBloc});

  @override
  State<PurchaseTransactionReportPage> createState() =>
      _PurchaseTransactionReportPageState();
}

class _PurchaseTransactionReportPageState
    extends State<PurchaseTransactionReportPage>
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      if (state.hasMorePurchaseTransaction &&
          state.status !=
              PurchaseOrderStatus.loadingMorePurchaseTransactionReport) {
        context.read<PurchaseOrderBloc>().add(
  ...
  void _showFilterDialog() async {
    ...
    if (result != null) {
      context.read<PurchaseOrderBloc>().add(
        UpdatePurchaseTransactionReportFilters(result),
      );
    }
  }
  ...
  void _exportToExcel() {
    final state = context.read<PurchaseOrderBloc>().state;
    context.read<PurchaseOrderBloc>().add(
      ExportPurchaseTransactionReportToExcel(state.purchaseTransactionFilters),
    );
  }
  ...
  void _exportToPDF() {
    final state = context.read<PurchaseOrderBloc>().state;
    context.read<PurchaseOrderBloc>().add(
      ExportPurchaseTransactionReportToPDF(state.purchaseTransactionFilters),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Purchase Transaction Report'),
  ...
  Widget _buildTransactionList(
    ...
    if (state.status == PurchaseOrderStatus.loading &&
        state.purchaseTransactionReports.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    }

    final items = state.purchaseTransactionReports;
    ...
    return ListView.separated(
      ...
      itemCount: items.length + (state.hasMorePurchaseTransaction ? 1 : 0),
  ...
  Widget _buildAvatar(PurchaseOrderHeader item) {
    ...
    final paymentType = item.paymentTerm;
    if (paymentType != null) {
  ...
  Widget _buildStatusBadge(PurchaseOrderHeader item) {
    final purchaseType = item.paymentTerm;
    return Container(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          _buildDetailRow(
            'Grand Total Amount',
            _currencyFormat.format(header.amountGrandTotalCost ?? 0),
            Iconsax.chart_2,
            valueColor: const Color(0xFF1C4292),
          ),
          ...
          _buildDetailRow(
            'Purchase Type',
            header.paymentTerm != null ? 'Credit' : 'Cash',
            Iconsax.chart_2,
          ),
  ...
  Widget _buildSummaryCards(
    ...
    final totals = state.purchaseTransactionTotals;
              ...
              children: [
                Expanded(
                  child: _buildSummaryItem(
                    'Total Amount Gross',
                    _currencyFormat.format(totals.totalAmountGross),
                    Iconsax.money_send,
                    Colors.green,
                  ),
                ),
                Expanded(
                  child: _buildSummaryItem(
                    'Total Grand Gross Amount',
                    _currencyFormat.format(totals.totalGrandAmountGross),
            ...
            Row(
              ...
              children: [
                Expanded(
                  child: _buildSummaryItem(
                    'Page',
                    '${state.purchaseTransactionPage}/${state.purchaseTransactionTotalPages}',
                    Iconsax.document,
                    Colors.blue,
                  ),
                ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              _buildFilterDescription(state.purchaseTransactionFilters),

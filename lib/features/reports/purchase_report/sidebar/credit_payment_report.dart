...
class CreditPaymentReport extends StatefulWidget {
  final AuthBloc authBloc;
  const CreditPaymentReport({super.key, required this.authBloc});

  @override
  State<CreditPaymentReport> createState() => _CreditPaymentReportState();
}

class _CreditPaymentReportState extends State<CreditPaymentReport>
  ...
  void _onScroll() {
    if (_scrollController.position.pixels ==
      ...
      if (state.hasMoreCreditPayment &&
        ...
        context.read<PurchaseOrderBloc>().add(LoadMoreCreditPaymentReport());
      }
    }
  }
  ...
  void _showFilterDialog() async {
    ...
    if (result != null) {
      context.read<PurchaseOrderBloc>().add(
        UpdateCreditPaymentReportFilters(result),
      );
    }
  }

  void _showItemDetail(CreditPayment item) {
    setState(() {
      _selectedItem = item;
      _itemDetail = true;
    });
  ...
  void _exportToExcel() {
    final state = context.read<PurchaseOrderBloc>().state;
    context.read<PurchaseOrderBloc>().add(
      ExportCreditPaymentReportToExcel(state.creditPaymentFilters),
    );
  }
  ...
  void _exportToPDF() {
    final state = context.read<PurchaseOrderBloc>().state;
    context.read<PurchaseOrderBloc>().add(
      ExportCreditPaymentReportToPDF(state.creditPaymentFilters),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Credit Payment Report'),
  ...
  Widget _buildSummaryCard(PurchaseOrderState state) {
    final totalAmount = state.creditPaymentReports.fold<double>(
      0.0,
      (previousValue, element) =>
          previousValue + (element.paymentAmount ?? 0.0),
    );
          ...
          children: [
            Row(
              ...
              children: [
                _buildSummaryItem(
                  'Total items',
                  '${state.creditPaymentReports.length}',
                ...
                _buildSummaryItem(
                  'Page',
                  '${state.creditPaymentPage}/${state.creditPaymentTotalPages}',
                  Iconsax.document,
                  Colors.blue,
                ),
              ],
            ...
            if (state.status ==
                PurchaseOrderStatus.loadingMoreCreditPaymentReport)
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              _buildFilterDescription(state.creditPaymentFilters),
          ...
          IconButton(
            ...
            onPressed: () {
              context.read<PurchaseOrderBloc>().add(
                ClearCreditPaymentReportFilters(),
              );
            },
          ),
        ],
  ...
  String _buildFilterDescription(PurchaseReportFilters filters) {
    ...
    if (filters.supplierId != null) {
      parts.add('Supplier: ${filters.supplierId}');
    }

    if (filters.dateFrom != null && filters.dateTo != null) {
      parts.add('Date Range: ${filters.dateFrom} - ${filters.dateTo}');
    }

    if (filters.purchaseType != null) {
      parts.add('Purchase Type: ${filters.purchaseType}');
    }
  ...
  Widget _buildLotList(PurchaseOrderState state) {
    ...
    if (state.status == PurchaseOrderStatus.loadingCreditPaymentReport &&
        state.creditPaymentReports.isEmpty) {
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('Loading Credit Payment report...'),
          ],
        ),
      );
    }
          ...
          children: [
            ...
            Text(
              state.error!.isNotEmpty
                  ? state.error!
                  : 'Failed to load Credit Payment report',
    ...
    if (state.creditPaymentReports.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'No credit payment report found',
    ...
    return Container(
      ...
      child: ListView.separated(
        ...
        itemCount:
            state.creditPaymentReports.length +
            (state.creditPaymentReports.isEmpty ? 1 : 0),
        separatorBuilder: (context, index) => SizedBox(height: cardSpacing),
        itemBuilder: (context, index) {
          if (index >= state.creditPaymentReports.length) {
            return Padding(
              padding: const EdgeInsets.all(16),
              child: Center(
                child: Column(
          ...
          final lot = state.creditPaymentReports[index];
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                Text(
                                  item.poHeaderRef?.supplierRef?.supplierName ??
                                      'N/A',
                                  style: TextStyle(
                            ...
                            if (item.poHeaderRef?.orderTypeRef?.description1 !=
                                null)
  ...
  Widget _builditemDetailContent(CreditPayment item, bool isCompact) {
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          _buildDetailItem(
            'Supplier: ',
            item.poHeaderRef?.supplierRef?.supplierName ?? 'N/A',
            Iconsax.user,
            isCompact,
          ),
          _buildDetailItem(
          ...
          _buildDetailItem(
            'Transaction Ref: ',
            'CO - ${item.poHeaderRef?.orderNumber ?? 'N/A'}',
            Iconsax.document,
            isCompact,
          ),
          _buildDetailItem(
            'Invoice: ',
            item.poHeaderRef?.invoiceNumber ?? 'N/A',
            Iconsax.document,
            isCompact,
          ),
          _buildDetailItem(
  ...
  Widget _buildLotAvatar(CreditPayment item, bool isCompact) {
    return Container(
      width: 48,
      height: 48,
      decoration: BoxDecoration(
  ...
  Widget _builditemDescription(CreditPayment item) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text(
            item.poHeaderRef?.orderTypeRef?.description1 ?? 'N/A',
            style: TextStyle(
              fontSize: 10,
              color: Colors.blue,
              fontWeight: FontWeight.w600,

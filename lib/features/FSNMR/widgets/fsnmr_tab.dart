...
class FSNMRTab extends StatefulWidget {
  final AuthBloc authBloc;
  const FSNMRTab({super.key, required this.authBloc});

  @override
  State<FSNMRTab> createState() => FSNMRTabState();
}

class FSNMRTabState extends State<FSNMRTab> {
  ...
  Future<void> navigateToCreateRule() async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => FSNMRCreateAndEditPage(authBloc: widget.authBloc),
      ),
    );
    if (result == true) {
      _loadInitialData();
  ...
  Future<void> _navigateToEditRule(FastSlowNonMovingRule item) async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) =>
            FSNMRCreateAndEditPage(authBloc: widget.authBloc, rule: item),
      ),
    );
    if (result == true) {
      _loadInitialData();
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<FSNMRBloc, FSNMRState>(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (_searchController.text.isEmpty)
                                ElevatedButton.icon(
                                  onPressed: navigateToCreateRule,
